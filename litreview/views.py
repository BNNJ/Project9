...
def edit_ticket(request, ticket_id):
    ...
    return render(request, "litreview/ticket/edit_ticket.html", {"form": form})
...
def create_review(request):
    ...
    if request.method == "POST":
        ...
        review_form = modelform_factory(
            Review, exclude=["ticket", "user"], widgets={"rating": RadioSelect}
        )(prefix="rform")
    return render(
        request,
...
def feed(request):
    ...
    tickets = Ticket.objects.filter(user__in=[*followed_users, request.user]).annotate(
    ...
    reviews = reviews.filter(
        Q(user__in=[*followed_users, request.user]) | Q(ticket__in=user_tickets)
...
def follow(request):
    ...
    followed = [f.followed_user for f in users.filter(user=request.user)]
    followers = [f.user for f in users.filter(followed_user=request.user)]
...
def unfollow(request, user_id):
    f = UserFollows.objects.filter(user=request.user).filter(followed_user=user_id)
    if f:
        f.delete()
    else:
